...
- **拖尾方向**：两个独立的坑，都表现为"拖尾方向乱偏"——① 残影回叠（见上条）；② Pro 星云彗尾必须沿「上一帧真实位置 p.lx/p.ly → 当前位置」反向延长，不能按轨道角度推算尾起点（Pro 粒子半径随能量外推逐帧跳动，角度推算会脱离真实轨迹；经典星云一直用 lx/ly 所以不偏）。修复后彗尾长度 = 单帧位移 × (1.6+v×4)，静止时（位移<0.3px）不画尾。
